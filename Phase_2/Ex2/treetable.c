...
enum cc_stat treetable_get_greater_than(TreeTable const * const table, const void *key, void **out)
{
    ...
    //sem isto, caso nao haja nó sucessor, ele retorna cc_ok
    //
    if (s != table->sentinel && n && s) {
        *out = s->key;
        return CC_OK;
    }
    return CC_ERR_KEY_NOT_FOUND;
...
static INLINE RBNode *tree_min(TreeTable const * const table, RBNode *n)
{
    ...
    while (n != s && n->left != s) //se a arvore for vazia, n = sentinel, por isso n->left não existe
        n = n->left;
...
static int height(TreeTable* t, RBNode* node) {
    
    if (node == t->sentinel) 
        return 0;
    
    int leftHeight = height(t, node->left);
    int rightHeight = height(t, node->right);

    return leftHeight > rightHeight? leftHeight + 1 : rightHeight + 1;
}

int balanced(TreeTable* t) {
    return abs(height(t, t->root->left) - height(t, t->root->right)) <= 1;
}

static int isSorted(TreeTable* t, RBNode* node) {
    ...
    RBNode* maxLeft = tree_max(t, node->left);
    RBNode* minRight = tree_min(t, node->right);

    if (!(maxLeft != t->sentinel && node->key > maxLeft->key) &&
        (minRight != t->sentinel &&  node->key < minRight->key)) {
        return 0; 
    }
    
    return isSorted(t, node->left) && isSorted(t, node->right);
}

int sorted(TreeTable* t) {
    return isSorted(t, t->root);
}